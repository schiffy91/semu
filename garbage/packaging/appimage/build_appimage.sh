...
[ -z "$ESDE_APPIMAGE" ] && ESDE_APPIMAGE="$REPO_ROOT/src/generated/packaging/es-de/ES-DE.AppImage"
[ -z "$OUTPUT" ] && OUTPUT="$REPO_ROOT/src/generated/packaging/linux/Semu-${ARCH}.AppImage"
mkdir -p "$(dirname "$OUTPUT")"

if [ ! -f "$ESDE_APPIMAGE" ]; then
  echo "ES-DE AppImage not found at $ESDE_APPIMAGE" >&2
  echo "Pass --esde-appimage <path> or place one at src/generated/packaging/es-de/ES-DE.AppImage." >&2
  exit 2
fi
...
if [ -z "$NIX_PACKAGE" ] && [ -e "$REPO_ROOT/src/generated/nix/result" ]; then
  NIX_PACKAGE="$REPO_ROOT/src/generated/nix/result"
fi
...
mkdir -p "$APPDIR/src/semu/packaging/appimage"
cp -r "$HERE/." "$APPDIR/src/semu/packaging/appimage/"
# Don't ship the build script itself inside.
rm -f "$APPDIR/src/semu/packaging/appimage/build_appimage.sh"

if [ -d "$REPO_ROOT/src/generated/packaging/es-de" ]; then
  mkdir -p "$APPDIR/src/generated/packaging"
  cp -r "$REPO_ROOT/src/generated/packaging/es-de" "$APPDIR/src/generated/packaging/"
fi

if [ -d "$REPO_ROOT/src/semu/packaging/templates" ]; then
  mkdir -p "$APPDIR/src/semu/packaging"
  cp -r "$REPO_ROOT/src/semu/packaging/templates" "$APPDIR/src/semu/packaging/"
fi
...
if [ ! -x "$APPDIR/usr/bin/semu" ] && [ -x "$REPO_ROOT/src/generated/build/semu" ]; then
  cp "$REPO_ROOT/src/generated/build/semu" "$APPDIR/usr/bin/semu"
  chmod +x "$APPDIR/usr/bin/semu"
...
cp "$HERE/AppRun.template" "$APPDIR/AppRun"
...
cp "$HERE/semu.desktop.template" "$APPDIR/semu.desktop"
