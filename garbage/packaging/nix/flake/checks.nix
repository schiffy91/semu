...
forAllSystems (system: let
      ...
      mkMockRoutedEmulator = spec:
        pkgs.callPackage ../semu_emulators.nix ({
      ...
      in lib.nameValuePair "routed-emulator-${spec.emulatorName}-mock" (pkgs.runCommand "semu-routed-emulator-${spec.emulatorName}-mock-check" {
        ...
        profiles="$project/src/generated/packaging/emulators/profiles"
        ...
        state="$project/src/generated/runtime/appimage-state/${spec.emulatorName}"
        grep -F "COMMAND=${spec.commandName}" "$capture"
        grep -F "SEMU_PROJECT_DIR=$project" "$capture"
        grep -F "SEMU_ROMS_DIR=$project/src/generated/runtime/content/ROMs" "$capture"
