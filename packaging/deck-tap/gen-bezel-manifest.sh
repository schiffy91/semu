...
#
# Emits:
#   packaging/deck-tap/bezels.json           canonical, human-readable (the source of record)
#   src/semu/emulators/bezel_manifest.btrc   generated; auto-imported (./semu/emulators/*) and
#                                            consumed by launcherTapScreen via bezelManifestHole()
#
...
JSON="$REPO/packaging/deck-tap/bezels.json"
BTRC="$REPO/src/semu/emulators/bezel_manifest.btrc"
tmpj="$(mktemp)"; tmpb="$(mktemp)"
echo "{" > "$tmpj"
{
  echo "// GENERATED by packaging/deck-tap/gen-bezel-manifest.sh — DO NOT EDIT BY HAND."
  echo "// Per-bezel TV-glass rect (x,y,w,h normalized, top-left), auto-measured + aspect-snapped."
  echo "// Regenerate after adding/changing bezel art. Consumed by launcherTapScreen()."
  echo "string bezelManifestHole(string system) {"
  echo "    string s = system.toLower();"
} > "$tmpb"
...
for row in "${TABLE[@]}"; do
  ...
  printf '    if (s.equals("%s")) { return "%s,%s,%s,%s"; }\n' "$sys" "$hx" "$hy" "$hw" "$hh" >> "$tmpb"
  count=$((count+1))
done

echo "" >> "$tmpj"; echo "}" >> "$tmpj"
{ echo '    return "";'; echo "}"; } >> "$tmpb"
mv "$tmpj" "$JSON"; mv "$tmpb" "$BTRC"
echo "wrote $count bezel(s) -> $JSON + $BTRC"
