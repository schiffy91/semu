#!/usr/bin/env bash
# gen-bezel-manifest.sh — auto-measure every bezel's TV-glass rect and emit the manifest.
#
...
#
# Emits:
#   config/assets/bezels.json                canonical, human-readable source of record
#
...
JSON="$REPO/config/assets/bezels.json"
tmpj="$(mktemp)"
echo "{" > "$tmpj"
...
# gen-handheld-shells.sh. The compositor fills this hole at the display aspect (SEMU_TAP_FILL), so the
