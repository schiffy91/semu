...
Example:
  curl -fsSL https://raw.githubusercontent.com/schiffy91/semu/refs/heads/main/src/semu/packaging/installer.sh \
...
default_roms_dir() {
  ...
  for candidate in \
    ...
    /run/media/deck/*/ROMs \
    "$PROJECT/src/generated/runtime/content/ROMs"; do
    [ -d "$candidate" ] && { printf '%s\n' "$candidate"; return; }
  done
  printf '%s\n' "$PROJECT/src/generated/runtime/content/ROMs"
}
...
build_semu() {
  ...
  (
    ...
    mkdir -p src/generated/nix
    if [ "$BUILD_MODE" = "full" ]; then
      log 'build full Semu bundle'
      nix build --out-link src/generated/nix/result .#default
    else
      log 'build Semu CLI package'
      nix build --out-link src/generated/nix/result .#semu-cli
    fi
  )
}

semu_bin() {
  for candidate in "$PROJECT/src/generated/build/semu" "$PROJECT/src/generated/nix/result/bin/semu"; do
