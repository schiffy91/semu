...
static float lcd_mono_amt = 0.0f, lcd_wash_amt = 0.0f;                    // handheld LCD: DMG monochrome green ramp + reflective wash
...
static GLint uRect2, uSrc2, uDual, uAlign, uHole, uGlass, uHasGlass, uReflect, uCurve, uScreenCorner, uLcdGrid, uLcdSub, uLcdMono, uLcdWash, uMenuRect, uMenuOn;
static long frames;
...
static const char *FS =
 ...
 "        vec2 cc=uv*2.0-1.0; float m=1.0-smoothstep(-0.012,0.012, sdRound(cc, vec2(1.0), clamp(uScreenCorner,0.0,0.5)));\n"  // rounded screen corners match the art opening
 "        oc=mix(oc,g,m);\n"                                        // blend so the bezel's rounded frame shows through the corners
 "      }\n"
 "    }\n"
...
static void gl_init(void) {
    ...
    uGlass=p_uloc(prog,"uGlass"); uHasGlass=p_uloc(prog,"uHasGlass"); uReflect=p_uloc(prog,"uReflect"); uCurve=p_uloc(prog,"uCurve"); uScreenCorner=p_uloc(prog,"uScreenCorner"); uLcdGrid=p_uloc(prog,"uLcdGrid"); uLcdSub=p_uloc(prog,"uLcdSub"); uLcdMono=p_uloc(prog,"uLcdMono"); uLcdWash=p_uloc(prog,"uLcdWash");
...
static SemuTapMenuState menu_state(void){
    ...
    s.priority_mode=priority_mode; s.bezel_index=bezel_idx; s.bezel_count=bezel_count; s.shader_index=shader_idx;
...
static void menu_apply_state(const SemuTapMenuState *s){
    if(!s)return;
    menu_lvl=s->level; menu_sel=s->selected; priority_mode=s->priority_mode; bezel_idx=s->bezel_index; shader_idx=s->shader_index;
...
// dlvsym is glibc-only; this prototype exists so the darwin `gcc -fsyntax-only` gate can
// parse this translation unit. The tap itself only builds/loads on x86_64-linux (glibc).
...
static void tap_init(void) {
    ...
    const char *lm=getenv("SEMU_TAP_LCD_MONO");     if(lm) lcd_mono_amt=(float)atof(lm);                          // DMG monochrome green ramp
    const char *lw=getenv("SEMU_TAP_LCD_WASH");     if(lw) lcd_wash_amt=(float)atof(lw);                          // reflective panel wash
...
static void tap_frame(void *dpy, unsigned long drawable, int is_egl) {
    ...
    if (!disabled && p_copytex && p_createsh && p_draw && p_use && g_have && g_state.active && g_state.abi == SEMU_TAP_ABI) {
        ...
        if (w>0 && h>0) {
            ...
            geom_in.priority_bezel=(priority_mode==2||priority_mode==3); geom_in.fill_hole=fill_hole||(priority_mode==3);
            ...
            if(dual_mode){
                ...
                } else if(priority_mode==1){
                    // GAME (TOP SCREEN) PRIORITY: the top screen takes the largest integer
                    // scale that still leaves ~20%% of the axis for the bottom screen; the
                    // bottom aspect-fits whatever remains (fractional is fine there).
                    int gap=8,k;
                    if(nds_layout==0){
                ...
                { int hv=bezel_idx; if(hv>=bezel_count) hv=(bezel_count>0?bezel_count-1:0); if(hv<0) hv=0;
                  if((priority_mode==2||priority_mode==3) && dual_hole_set[hv]){
                    int nhp=(nh>=288)?nh/2:nh;
                    // BEZEL priority, dual: size the ART from the PRIMARY hole with the art
                    // aspect preserved (the union hole spans the hinge gap and would distort
                    // it): largest integer k where primary == k x native and the art is uncut.
                    { float art_aspect=(ahv>0)?((float)awv/(float)ahv):1.6f; int kk=1,best=0;
                      for(;;){ float cand_w=(float)(kk*nw)/pri_rects[hv][2]; float cand_h=cand_w/art_aspect;
                               if(cand_w<=(float)w*1.01f+0.5f && cand_h<=(float)h*1.01f+0.5f){ best=kk; kk++; } else break; }
                      if(best>=1){ bdw=(float)(best*nw)/pri_rects[hv][2]; bdh=bdw/art_aspect;
                                   bdx_gl=((float)w-bdw)*0.5f; bdy_gl=((float)h-bdh)*0.5f; } }
                    // BEZEL priority: largest INTEGER per-screen scale that fits each hole
                    // (floor, clamped >=1); FIT (mode 3): fractional aspect-fit filling the hole.
                    ...
                    float fw,fh;
                    // FIT (mode 3): fill the hole exactly (holes are authored to each
                    // screen's native aspect, so no distortion). BEZEL: largest integer.
                    if(priority_mode==3){ fw=hw2; fh=hh2; }
                    else { int k=(int)(hw2/(float)nw); int kh=(int)(hh2/(float)nhp); if(kh<k) k=kh; if(k<1) k=1;
            ...
            if(uLcdMono>=0)p_u1f(uLcdMono,lcd_mono_amt);
            if(uLcdWash>=0)p_u1f(uLcdWash,lcd_wash_amt);
            ...
            if(++frames<=2 || (menu_on && (frames%30)==0)){ FILE *lf=tap_fopen("semutap.log","a"); if(lf){ fprintf(lf,"frame%ld: fb=%dx%d native=%dx%d content=(%d,%d,%d,%d) pri=%d game=(%d,%d,%d,%d) bd=(%d,%d,%d,%d) menu_on=%d effArt=%.1f bidx=%d bcnt=%d uMenuOn=%d uHasArt=%d\n",frames,w,h,nw,nh,g_state.content_x,g_state.content_y,g_state.content_w,g_state.content_h,priority_mode,gx,gy,gw,gh,(int)bdx_gl,(int)bdy_gl,(int)bdw,(int)bdh,menu_on,eff_has_art,bezel_idx,bezel_count,(int)(uMenuOn>=0),(int)(uHasArt>=0)); fclose(lf);} }
